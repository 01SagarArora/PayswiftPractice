...
const chunkExtractor = new ChunkExtractor({ statsFile });
  ...
  if (IS_DEV) {
    ...
    https.createServer(options, app).listen(DEV_HTTPS_SERVER, () => {
      logger.info(`Application HTTPS server started on port ${DEV_HTTPS_SERVER} and evn is ${process.env.NODE_ENV} `);
    });
  }

  server.listen(PORT, () => {
    logger.info(`Application Client listening on port ${PORT}! (render to ${IS_RENDER_TO_STREAM ? 'stream' : 'string'}) and evn is ${process.env.NODE_ENV}`);
  });
};
